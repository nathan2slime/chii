import { Session, User } from '@chi/database/store'
...
export class SessionService {
  constructor(
    ...
    private readonly dbStore: StorePrismaService,
    private readonly configService: ConfigService<AppConfig>
  ) {}

  async create(user: Omit<User, 'password'>) {
    const newSession = await this.dbStore.prisma.session.create({
      data: {
        user: {
          connect: {
            id: user.id
    ...
    const data = await this.dbStore.prisma.session.update({
      where: {
        id: newSession.id
      },
      data: {
  ...
  async expire(id: string) {
    this.logger.info('expire session in this.redis', id)
    await this.redis.client.del(this.getCacheKey(id))

    this.logger.info('removing session', id)
    await this.dbStore.prisma.session.delete({
      where: { id }
    })
  }
  ...
  async deleteAllByUserId(userId: string) {
    const sessions = await this.dbStore.prisma.session.findMany({
      where: { userId },
      select: {
        id: true
      }
    ...
    return this.dbStore.prisma.session.deleteMany({ where: { userId } })
  }
  ...
  async findById(id: string) {
    ...
    const data = await this.dbStore.prisma.session.findUnique({
      where: { id },
      include: { user: true }
    })
